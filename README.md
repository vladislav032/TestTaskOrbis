...
### 1. **Сколько времени заняла реализация приложения?**  
На реализацию приложения ушло примерно:  
- **Базовая структура и маршруты** — 3–5 часов  
- **Настройка базы данных и моделей** — 1–2 часа  
- **Обработка файлов (загрузка, удаление, изменение)** — 2–5 часа  
- **Синхронизация и устранение дубликатов** — 3–4 часа  
- **Тестирование и отладка** — 4–5 часа  

 **Общая оценка времени:** ~2 дня работы  

---

### 2. **Какой технологический стек используется в реализации?**  
...
| Формат ответов | JSON |
| Потоки | Модуль `threading` для синхронизации |

---

### 3. **Какие типовые решения были использованы в реализации?**  

 **Архитектурные шаблоны:**  
- **MVC (Model-View-Controller)** — отделение логики представления (routes), модели (models) и логики обработки файлов (file_handler).  
- **Клиент-серверная архитектура** — обработка HTTP-запросов и управление данными.  
- **Репозиторий данных** — через SQLAlchemy для управления объектами в базе данных.  

 **Шаблоны проектирования:**  
- **Singleton** — подключение к базе данных через `SessionLocal` создаётся как единый экземпляр.  
- **Data Mapper** — ORM через SQLAlchemy для преобразования данных между объектами Python и базой данных.  
- **Observer** — поток синхронизации реализует отслеживание изменений в файловой системе.  
- **Command** — операции с файлами (загрузка, удаление) реализованы как команды в `file_handler.py`.  
- **Flyweight** — путь к файлу повторно используется в синхронизации и проверке дубликатов.  
- **Factory** — создание объекта `FileRecord` в обработчиках загрузки и синхронизации.  

 **Шаблоны работы с базой данных:**  
- **CRUD** — стандартные операции создания, чтения, обновления и удаления реализованы в маршрутах.  
- **Repository Pattern** — работа с базой данных через сессии SQLAlchemy.  

 **Шаблоны обработки файлов:**  
- **Command** — операции с файлами реализованы в виде команд.  
- **Flyweight** — повторное использование пути к файлу в логике синхронизации и проверки дубликатов.  
